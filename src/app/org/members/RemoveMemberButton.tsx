...
export function RemoveMemberButton({ userId, self, onDone }: { userId: string; self: boolean; onDone: ()=>void }) {
  ...
  async function submit() {
    ...
    try {
  const res = await fetch('/api/org/members', { method:'DELETE', headers:{'content-type':'application/json'}, body: JSON.stringify({ user_id: userId }) });
      ...
      {open && (
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,.35)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000}}>
          <div className="card stack" style={{maxWidth:480}}>
            <h3>Remove member</h3>
            <p style={{fontSize:14,lineHeight:1.4}}>Cette action retire ce membre de l'organisation. Les invitations en attente associées à son email seront supprimées. Le compte utilisateur reste actif et pourra être réinvité.</p>
